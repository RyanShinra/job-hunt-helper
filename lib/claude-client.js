...
const ClaudeClient = {
   ...
   */
  async analyzeJob(apiKey, jobData, resumeText = null) {
    if (!apiKey) {
    ...
    try {
      const prompt = this.buildAnalysisPrompt(jobData, resumeText);
   ...
   */
  buildAnalysisPrompt(jobData, resumeText = null) {
...
**Match Score**
Rate 1-10 with clear reasoning${safeResume ? ' based on the candidate\'s background' : ' for a typical candidate at this level'}.
