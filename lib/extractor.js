...
const JobExtractor = {
  ...
  async extractLinkedIn() {
    try {
      ...
      // FALLBACK: If job title is empty, try to find it from any h1
      if (!jobTitle || jobTitle.trim().length === 0) {
        console.log('🔍 FALLBACK: Job title empty, checking all h1 elements...');
        const h1Elements = Array.from(document.querySelectorAll('h1'))
          .filter(h1 => {
            const text = h1.textContent?.trim() || '';
            // Likely job titles: 20-150 chars, not navigation text
            return text.length > 10 && text.length < 150 && !text.toLowerCase().includes('linkedin');
          })
          .sort((a, b) => (b.textContent?.length || 0) - (a.textContent?.length || 0));

        if (h1Elements.length > 0) {
          jobTitle = h1Elements[0].textContent?.trim() || '';
          console.log(`  ✅ FALLBACK: Using h1 text as job title: "${jobTitle.substring(0, 60)}..."`);
        }
      }
