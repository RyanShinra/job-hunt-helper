...
const JobExtractor = {
  ...
  /**
   * Detects which LinkedIn page layout we're on
   * @returns {string} 'collections' or 'direct' or 'unknown'
   */
  detectLinkedInPageType() {
    const url = window.location.href;

    // Collections view: /jobs/collections/...?currentJobId=XXX
    if (url.includes('/jobs/collections/') || url.includes('currentJobId=')) {
      return 'collections';
    }

    // Direct view: /jobs/view/XXXXX/
    if (url.includes('/jobs/view/')) {
      return 'direct';
    }

    // Other LinkedIn jobs pages
    return 'unknown';
  },
  ...
  async waitForLinkedInRender(maxAttempts = 20, delay = 500) {
    for (let attempt = 1; attempt <= maxAttempts; attempt++) {
      ...
      console.log('  Page readiness:', { hasH1, hasH2, hasH3, hasJobCard, hasDescription, hasScaffold, hasJobDetails });

      // Collections view often has h2/h3 but no h1, so accept those too
      if (hasH1 || hasH2 || hasH3 || hasJobCard || hasDescription || hasScaffold || hasJobDetails) {
  ...
  async extractLinkedIn() {
    try {
      const pageType = this.detectLinkedInPageType();
      console.log('Job Hunt Assistant: Starting LinkedIn extraction');
      console.log('  URL:', window.location.href);
      console.log('  Page type:', pageType, pageType === 'collections' ? '(sidebar + job detail)' : pageType === 'direct' ? '(single job page)' : '');

      // Wait for LinkedIn to finish rendering
      await this.waitForLinkedInRender();

      // DEBUG: Log all h1 elements on the page (or h2/h3 if no h1)
      const allH1s = document.querySelectorAll('h1');
      const allH2s = document.querySelectorAll('h2');
      const allH3s = document.querySelectorAll('h3');

      console.log('🔍 DEBUG: Found', allH1s.length, 'h1 elements,', allH2s.length, 'h2 elements,', allH3s.length, 'h3 elements');
      ...
      if (allH1s.length > 0) {
        ...
        // No h1s, show h2s and h3s instead
        console.log('  No h1 elements found. Showing h2/h3 elements:');
        allH2s.forEach((h2, index) => {
          if (index < 5) { // Limit to first 5
            console.log(`  H2 #${index}:`, {
              text: h2.textContent?.substring(0, 60),
              classes: h2.className.substring(0, 50),
              id: h2.id
            });
          }
        });
        allH3s.forEach((h3, index) => {
          if (index < 5) { // Limit to first 5
            console.log(`  H3 #${index}:`, {
              text: h3.textContent?.substring(0, 60),
              classes: h3.className.substring(0, 50),
              id: h3.id
            });
          }
        });
      ...
      const jobTitleSelectors = [
        // Collections view - new layout (h2/h3/p based, not h1)
        'h2[class*="job"]',
        'h3[class*="job"]',
        'p[class*="job"]',
        '[class*="job-title"] h2',
        '[class*="job-title"] h3',
        '[class*="job-title"] p',
        // Jobs collection/sidebar view (older layout)
