...
const Storage = {
  ...
  /**
   * Saves resume text to storage
   * @param {string} resumeText - Resume text content
   * @returns {Promise<boolean>} Success status
   */
  async saveResume(resumeText) {
    try {
      const MAX_RESUME_LENGTH = 30000; // characters
      const truncatedResume = this.truncateText(resumeText, MAX_RESUME_LENGTH);
  ...
  /**
   * Retrieves resume text from storage
   * @returns {Promise<string|null>} Resume text or null
   */
  async getResume() {
    try {
  ...
  async deleteResume() {
    try {
      await chrome.storage.local.remove(['resume', 'resumeUploadedAt']);
   ...
   * @param {string} preferences - User career preferences and context
   * @returns {Promise<boolean>} Success status
   */
   ...
   * @returns {Promise<string|null>} Preferences text or null
   */
